...
class LockOverlay(private val context: Context) {
    ...
    private fun updateAppName() {
        ...
        val label = try {
            context.packageManager.getApplicationLabel(context.packageManager.getApplicationInfo(currentPackage, 0)).toString()
    ...
    private fun setupButtons(view: android.view.View) {
        ...
        listOf(R.id.btn0, R.id.btn1, R.id.btn2, R.id.btn3, R.id.btn4, 
               R.id.btn5, R.id.btn6, R.id.btn7, R.id.btn8, R.id.btn9).forEachIndexed { index, id ->
            view.findViewById<Button>(id).setOnClickListener {
                if (enteredPin.length < 8) {
                    enteredPin += index.toString()
                    updateDots(tvDots)

                    val target = if (recoveryMode) 8 else prefManager.pinLength
                    
                    // SECURITY GUARD: Enforce Fail-Closed logic
                    // Ensure the length requirements are met and PIN is not blank before validation
    ...
    private fun updateDots(tvDots: TextView) {
        val total = if (recoveryMode) 8 else prefManager.pinLength.coerceAtLeast(4)
    ...
    private fun grantAccess(pkg: String) {
        LockService.pollPausedUntil = System.currentTimeMillis() + 3000L
